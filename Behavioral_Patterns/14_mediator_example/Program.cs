...
// Chat Room
// This version of the pattern does not need a list of Colleagues in the Mediator. 
// The delegate keeps a record of the methods on its chain. All other information stays in the colleague itself.

namespace _14_mediator_example
{
    public delegate void Callback(string message, string from);

    // The Mediator is in two parts: Interact handles the GUI;
    // Mediator itself sets up the threads and does the broadcast from the delegate (Send method)
    class Mediator
    {
        Callback Respond;
