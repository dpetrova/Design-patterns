...
namespace _20_interpreter_example
{
    // A general-purpose interpreter for any .NET API
    // Reads XML and executes the methods it represents
    // Assuming the XML specification is in a file called calc_winforms.xml
        ...
        public Mirror(string spec)
        {
            ...
            while (Reader.Read())
            {
                ...
                if (Reader.HasAttributes)
                {
                    ...
                    {
                        Reader.MoveToAttribute(i); InterpretCommands();
                    }
                }
                if (b) Pop();
            }
    ...
    // Handles an XML element. Creates a new object that reflects the XML element name.
